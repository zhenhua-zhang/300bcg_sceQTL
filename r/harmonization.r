...
#' Harmonize variants
harmonize_vars <- function(eqtl, gwas, eqtl_pval_col = "p_value", eqtl_pval = 0.1,
                           clump = TRUE, clump_kb = 50, clump_r2 = 0.8, clump_bfile = NULL,
                           do_proxy = TRUE, plink_bin = "plink", bcftools_bin = "bcftools") {
  ...
  gwas_tab <- lapply(gwas_path, function(path, .rsid, .pval) {
    outcome_info <- basename(path) %>%
      stringr::str_remove_all(".vcf.gz") %>%
      stringr::str_split("_", n = 3, simplify = TRUE)
  ...
  if (clump && !is.null(clump_bfile)) {
    ...
    hm_tab <- dplyr::summarise(clump_snps, clumped = {
      ...
      dplyr::right_join(hm_tab, by = c("outcome", "id.exposure", "exposure", "id.outcome", "SNP")) %>%
...
for (mode in mode_vec) {
  ...
  .tmp <- lapply(names(qtltab_list), function(pr_run_id) {
    tmp_tab <- qtltab_list[[pr_run_id]] %>%
      (function(dat) {
        top_egenes <- dplyr::filter(dat, !is.na(global_corrected_pValue)) %>%
    ...
    if (!file.exists(hm_save_to) || override) {
      ...
      hm_dat <- harmonize_vars(tmp_tab, pub_gwas_list, clump_bfile = eur_1kg_geno, plink_bin = "plink-quiet")
