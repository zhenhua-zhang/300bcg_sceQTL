...
estimate_enrichment <- function(opts) {
  qtl_tab <- load_eqtl_tab(opts$qtl_db) %>%
    dplyr::select(snp_chromosome, snp_position, snp_id, gene_name, global_corrected_pValue, p_value)

  # Population ranges
  all_snp_ranges <- qtl_tab %>%
    dplyr::group_by(snp_chromosome, snp_position, snp_id) %>%
    dplyr::summarize(gene_name = paste0(gene_name, collapse = "|"),
                     is_top_snp = any(!is.na(global_corrected_pValue) & p_value <= 5e-2)) %>%
    (function(e) {
      GenomicRanges::GRanges(
        seqnames = e$snp_chromosome,
  ...
  if (FALSE) any(stringr::str_split(SNPS, pattern == "\\|", simplify = TRUE) %in% total_snps)
  # Haplotype blocks
  total_snps <- qtl_tab$snp_id %>% unique()
  hb_tab <- data.table::fread(opts$blocks)
  hb_ranges <- hb_tab %>%
    dplyr::filter(opts$min_block_size <= KB, KB <= opts$max_block_size) %>%
  ...
  population_size <- length(hb_ranges)
  nr_suc_exp <- length(qtl_blocks) # Trial size is the number of haplotype blocks containing SNPs
  ...
  lapply(anno_ranges, function(per_range) {
    mos <- ifelse(toupper(per_range$annotype) == "GWAS", 1, opts$min_ov_size)
    trial_size <- IRanges::overlapsAny(hb_ranges, per_range$ranges, minoverlap = mos) %>% sum()
    nr_suc_obs <- IRanges::overlapsAny(qtl_blocks, per_range$ranges, minoverlap = mos) %>% sum()

    estres <- data.frame(
      annotype = per_range$annotype, celltype = per_range$celltype,
      data_source = per_range$data_source, log2od = NA, p_val = NA, obs_prob = NA, exp_prob = NA,
      expectation = nr_suc_exp, population = population_size, observation = nr_suc_obs,
      trials = trial_size)
...
p <- optparse::add_option(p, c("-m", "--min-block-size"), default = 0.5, type = "double", help = "The minimal size (kbp) of blocks used to estimate the enrichment. Default: %(default)s")
p <- optparse::add_option(p, c("-M", "--max-block-size"), default = 100, type = "double", help = "The maximum size (kbp) of blocks used to estimate the enrichment. Default: %(default)s")
