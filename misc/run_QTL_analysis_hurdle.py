...
def run_QTL_analysis(pheno_filename, anno_filename, geno_prefix, plinkGenotype, output_dir, window_size=250000,
    ...
    for feature_id in feature_list:
        ...
        ########################################################################################################################################################
            ...
            if(not np.isnan(zeroValue)):
                ##Do we want to do a logit / probit here on the phenotype?
                fglmm = FastScanner(phenotype_b.values, cov_matrix, utils.economic_qs(glmm.posteriori_covariance()), 0) ##This is a test, using LMM approximation for GLMM bernoulli
