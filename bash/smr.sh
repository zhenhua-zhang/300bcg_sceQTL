...
EOF
    ...
    smr --make-besd \
      --eqtl-flist $temp_dir/$celltype/$condition/esd_file_list.txt \
      --out $out_dir/sceqtl/$model/$celltype.$condition
  done
done
...
cmp_pool=(
  ...
  SLFN5:2022_COVID19Release7_HGI-A2-ALL-eur-leave23andme-20220403:CD4T:T0_LPS.vs.T0_RPMI
  SLFN5:2022_COVID19Release7_HGI-A2-ALL-eur-leave23andme-20220403:CD8T:T0_LPS.vs.T0_RPMI
)
...
for per_cmp in ${cmp_pool[@]}; do
  ...
  if [[ xx$per_cond != xx ]]; then
    eqtl_summary=$eqtl_summary.$per_cond
  fi
